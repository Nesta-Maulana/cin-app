...
class ShowUnit extends Component
{
    ...
    public function mount(UnitRepositoryInterface $repository)
    {
        $this->repository = $repository;
    }
    public function hydrate()
    {
        $this->repository = app(UnitRepositoryInterface::class);
    }
    ...
    public function render()
    {
       try {
            ...
            return view('livewire.master.show-unit', [
                'table' => $table,
            ]);
        } catch (\Exception $e) {
            $this->alert('error', 'Error fetching permissions: ' . $e->getMessage());
            return view('master.show-unit', [
                'table' => collect([]),
            ]);
        }
    }
