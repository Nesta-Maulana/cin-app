        ...
        <div class="col-md-9 mx-md-auto">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('bot.index') }}" class="btn p-0" title="Kembali">
                ...
                <div class="card-body">
                    <form action="{{ route('bot.store') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="mb-3 col-md-12">
                                ...
                                <input class="form-control" type="text" id="session_name" name="session_name"
                                    value="{{ old('session_name') }}" placeholder="Session Name" autofocus required />
                            </div>
                        </div>

                        <div class="row">
                            <div class=" mb-3 col-md-12">
                                <label for="name" class="form-label">Choose Tenant</label>
                                <select name="tenant_id" id="tenant_id" class="select2 form-control">
                                    <option value="0" selected>Central Domain</option>
                                    @foreach ($tenants as $tenant)
                                        <option value="{{ $tenant->id }}">
                                            {{ $tenant->name . ' - ' . $tenant->domain->domain }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="my-2">
                            ...
                            <a href="{{ route('bot.index') }}" class="btn btn-label-secondary">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
