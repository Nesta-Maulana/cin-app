            ...
            $.fn.extend({
                treed: function(o) {
                    ...
                    tree.find('li').has("ul").each(function() {
                        ...
                        branch.prepend("<i class='indicator glyphicon " + closedClass +
                            "'></i>");
            ...
            <div class="card">
                <div class="card-header border-bottom d-md-flex justify-content-md-between align-items-md-center">
                    ...
                    <div
                        ...
                        @else
                            ...
                            <div class="flex-wrap my-1">
                                <a href="#" class="btn btn-secondary add-new btn-label-primary"
                                    data-bs-toggle="modal" data-bs-target="#modalSelectedStatus">
                                    Update Status
                                </a>
                            </div>
                        @endif
                ...
                <div class="table-responsive">
                    <table class="table border-top">
                        ...
                        <tbody>
                            @forelse($table->whereNull('main_menu') as $key => $item)
                                @php
                                    $updateRoute = route('menu.update', $item->id);
                                @endphp

                                <tr wire:key="main-{{ $item->id }}" class="bg-info text-white">
                                    <td style="width: 2%">
                                        <i class="fa-solid {{ $item->icon }} fa-sm mx-2 fs-5"></i>
                                    </td>

                                    <td class="col-3">
                                        <div class="d-flex flex-column">
                                            <a href="#" class="text-white text-truncate">
                                                <span class="fw-semibold">
                                                    {{ $item->name }}
                                                </span>
                                            </a>
                                            <small class="text-white">/{{ $item->url }}</small>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            @can('update-menu')
                                                <a href="{{ route('menu.edit', $item->id) }}" class="action-btn"
                                                    title="edit">
                                                    <i class="text-white fa-solid fa-edit fa-sm me-2 fs-5"></i>
                                                </a>
                                            @endcan
                                            ...
                                            @can('delete-menu')
                                                <a href="javascript:;" class="action-btn" title="delete"
                                                    ...
                                                    <i class="text-white fa-solid fa-trash fa-sm mx-2 fs-5"></i>
                                                </a>
                                ...
                                @if ($item->subMenu->count() > 0)
                                    @foreach ($item->subMenu as $submenu)
                                        <tr wire:key="main-{{ $submenu->i }}">
                                            <td style="width: 2%">
                                                <i class="fa-solid {{ $submenu->icon }} fa-sm mx-2 fs-5"></i>
                                            </td>
                                            ...
                                            <td class="col-3">
                                                <div class="d-flex flex-column">
                                                    ...
                                                    <small class="text-muted">/{{ $submenu->url }}</small>
                                                </div>
                                            </td>

                                            <td class="col-3">
                                                @if ($submenu->permission_id)
                                                    {{ $submenu->permission->name }}
                                                @endif
                                            </td>
                                            <td class="col-3">
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    @can('update-menu')
                                                        <a href="{{ route('menu.edit', $submenu->id) }}" class="action-btn"
                                                            title="edit">
                                                    ...
                                                    @can('delete-menu')
                                                        <a href="javascript:;" class="action-btn" title="delete"
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#modalDelete{{ $submenu->id }}">
        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    ...
                    <button wire:click="syncCache" class="btn btn-sm btn-label-primary" id="teamMemberList">
                            ...
                            <div wire:sortable="updateParentOrder" wire:sortable-group="updateChildGroup">
                                @foreach (getMenu() as $menu)
                                    <div class="ms-1 d-flex align-items-center text-body h6 mt-2"
                                        ...
                                        <i class="fa-solid {{ $menu->icon }} text-body me-2"></i>
                                        {{ $menu->name }}
                                    </div>
