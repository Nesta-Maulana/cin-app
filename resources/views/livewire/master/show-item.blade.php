            ...
            <div class="text-md-end text-start d-flex align-items-center justify-content-end flex-md-row flex-column">
                ...
                @can('create-item')
                    <div class="flex-wrap my-1">
                        <a href="{{ route('item.create') }}" class="btn btn-secondary text-white add-new btn-primary">
                            <span>
            ...
            <table class="table border-top">
                ...
                <tbody>
                    ...
                    @forelse($table as $key => $item)
                        @php
                            $updateRoute = route('item.update', $item->id);
                        @endphp
                        ...
                        <tr wire:key="row{{ $item->id }}">
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <a href="{{ route('item.show', $item->id) }}" class="action-btn" title="Detail">
                                        <i class="fa fa-eye fa-sm me-2 fs-5"></i>
                                    </a>

                                    @can('update-item')
                                        <a href="{{ route('item.edit', $item->id) }}" class="action-btn" title="edit">
