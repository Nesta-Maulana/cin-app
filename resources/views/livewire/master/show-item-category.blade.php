            ...
            <table class="table border-top">
                ...
                <tbody>
                    ...
                    @forelse($table as $key => $item)
                        @php
                            $updateRoute = route('item-category.update', $item->id);
                        @endphp
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    @can('update-item-category')
                                        <a href="{{ route('item-category.edit', $item->id) }}" class="action-btn"
                                            title="Edit / 编辑">
        ...
        <div class="card-body d-md-flex justify-content-md-between align-items-center pt-3 pb-2">
            <div class="align-self-start my-2 d-none d-md-block text-muted">
                <small>
                    ...
                    显示第 {{ $table->firstItem() }} 条到第 {{ $table->lastItem() }} 条，共 {{ $table->total() }} 条数据
                </small>
            </div>
            {{ $table->links() }}
        </div>
